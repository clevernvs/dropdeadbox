class DropBoxController {
    ...
    connectFirebase() {
        var firebaseConfig = {
            apiKey: "AIzaSyDLPI75Zmmp-yMLYU7dzh_sn-ElMaE1Wj0",
            authDomain: "dropbox-clone-784d8.firebaseapp.com",
            databaseURL: "https://dropbox-clone-784d8.firebaseio.com",
            projectId: "dropbox-clone-784d8",
            storageBucket: "dropbox-clone-784d8.appspot.com",
            messagingSenderId: "125028885424",
            appId: "1:125028885424:web:a1d5bcd8d79e8e2814ac75",
            measurementId: "G-E7E97YHMRS"
        };
    ...
    initEvents() {
        ...
        this.inputFilesEL.addEventListener('change', event => {
            ...
            uploadTask(event.target.files).then(responses => {

                responses.forEach(resp => {
                    this.getFirebaseRef().push().set({
                        name: resp.name,
                        type: resp.contentType,
                        path: resp.downloadURLa[0],
                        size: resp.size
                    });
                });
    ...
    uploadProgress(event, file) {
        ...
        let porcentUpload = parseInt((loaded / total) * 100);
        let timeleft = ((100 - porcentUpload) * timespent) / porcentUpload;

        this.progressBarEl.style.width = `${porcentUpload}%`;
        this.namefileEl.innerHTML = file.name;
        this.timeleftEl.innerHTML = this.formatTimeforHuman(this.timeleft);
    ...
    formatTimeforHuman(duration) {
        ...
        if (minutes > 0) { return `${minutes} minutos, ${seconds} segundos`; }
    ...
    renderNav() {
        ...
        for (let i = 0; i < this.currentFolder.length; i++) {
            ...
            if ((i + 1) === this.currentFolder.length) {
                ...
                span.className = 'breacrumb-segment__wrapper';
